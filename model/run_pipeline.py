...
from model.ingestion.load_data import load_raw_data
from model.cleaning.clean_data import clean_dataframe
from model.feature_engineering.build_features import build_features
...
def main():
    ...
    try:
        ...
        con.execute(f"CREATE OR REPLACE TABLE movies AS SELECT * FROM read_parquet('{parquet_path}')")
        ...
        print(f"Warning: DuckDB ingestion skipped due to file lock (the web server is likely running). Details: {e}")
