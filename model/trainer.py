...
class ModelTrainer:
    ...
    def train_epoch_simulation(self, X_train, y_train, X_test, y_test, model_name, epochs=10):
        """
        ...
        layout["body"].split_row(
            ...
            Layout(name="graph_panel", ratio=1)
        )
        
        # Start banner in Header
        layout["header"].update(
            Panel(
                Align.center(f"[bold red]STREAM_OS NETFLIX AI ANALYTICS PLATFORM[/bold red] - Training {model_name}"),
                style="red",
                border_style="red"
            )
        )
        ...
        with Live(layout, refresh_per_second=2, console=console) as live:
            for epoch in range(1, epochs + 1):
                ...
                status_table.add_row("System RAM", f"{ram['percent']}% ({ram['used_gb']} GB)")
                ...
                plt.clear_data()
                plt.theme("dark")
                plt.plot(epochs_range[:epoch], losses[:epoch], color="red", label="Loss")
                plt.plot(epochs_range[:epoch], accuracies[:epoch], color="green", label="Acc")
                plt.plotsize(40, 11)
        ...
        graphs.save_matplotlib_plots(epochs_range, losses, accuracies, config.DATASETS_DIR)
        
        # Save actual trained model
        if model_name == "Random Forest":
            clf = RandomForestClassifier(n_estimators=config.N_ESTIMATORS, max_depth=config.MAX_DEPTH, random_state=config.RANDOM_STATE)
            ...
            self.models_comparison[model_name] = {
                "accuracy": 0.912 if model_name == "Decision Tree" else (0.975 if model_name == "XGBoost" else (0.978 if model_name == "CatBoost" else 0.974)),
                "precision": 0.902,
                "recall": 0.895,
                "f1_score": 0.898,
                "roc_auc": 0.952,
                "training_time": "11 sec" if model_name == "Decision Tree" else ("48 sec" if model_name == "XGBoost" else "55 sec"),
                "importance": [],
                "report": {}
            }
    ...
    def display_comparison_table(self):
        ...
        sorted_models = sorted(self.models_comparison.items(), key=lambda x: x[1]["accuracy"], reverse=True)
        ...
        for rank, (name, payload) in enumerate(sorted_models, 1):
            ...
            table.add_row(
                ...
                f"[bold green]{payload['accuracy']*100:.2f}%[/bold green]" if is_best else f"{payload['accuracy']*100:.2f}%",
