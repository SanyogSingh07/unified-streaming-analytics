...
sys.path.append(os.path.join(model_dir, "preprocessing"))
...
"""
    console.print(
        Panel(Align.center(f"[bold red]{banner}[/bold red]"), border_style="red")
    )
...
def clean_dataset():
    ...
    with progress.get_training_progress_bar() as pg:
        task = pg.add_task(
            "[bold white]Parsing lists & cleaning dataset...[/bold white]", total=100
        )
...
def feature_engineering():
    ...
    with progress.get_training_progress_bar() as pg:
        task = pg.add_task(
            "[bold white]Extracting and mapping feature sets...[/bold white]", total=100
        )
...
def split_dataset():
    ...
    if df_features is None:
        raise ValueError(
            "df_features is not initialized. Run feature_engineering() first."
        )

    # Add target column is_hit
    df = df_features.with_columns(
        ((pl.col("Vote_Average") >= 7.0) & (pl.col("Vote_Count") >= 100))
        .cast(pl.Int32)
        .alias("is_hit")
    )
    ...
    for genre in sorted([g for g in unique_genres if g]):
        ...
        genre_exprs.append(
            pl.col("clean_genres")
            .str.contains(genre, literal=True)
            .cast(pl.Int32)
            .fill_null(0)
            .alias(col_name)
        )
    ...
    logger.log(
        f"Dataset split completed. Train size: {len(X_train)}, Test size: {len(X_test)}"
    )
...
def build_model():
    ...
    logger.log(
        f"Configuring Random Forest Ensembles with estimators={config.N_ESTIMATORS}, depth={config.MAX_DEPTH}"
    )


def train_model():
    # Train primary Random Forest model
    trainer.train_epoch_simulation(
        X_train, y_train, X_test, y_test, "Random Forest", config.EPOCHS
    )
...
def plot_training_graph():
    ...
    console.print(
        f"[bold yellow]Matplotlib evaluation plots saved successfully in {config.DATASETS_DIR}[/bold yellow]\n"
    )
...
def compare_models():
    ...
    console.print(
        "[bold white]Simulating benchmarking comparisons against other model variants...[/bold white]"
    )
...
def save_model():
    ...
    console.print(
        f"[bold green]✔ Optimized Random Forest model saved to: {config.MODEL_SAVE_PATH}[/bold green]\n"
    )
...
def generate_training_report():
    ...
    console.print(
        f"[bold green]✔ Ingested latest evaluations report to {results_path}[/bold green]"
    )
