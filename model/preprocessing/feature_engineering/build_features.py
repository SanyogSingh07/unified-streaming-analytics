...
def build_features(df: pl.DataFrame) -> pl.DataFrame:
    df = df.with_columns((pl.col("Vote_Count") / 100.0).alias("Popularity"))
    ...
    df = df.with_columns(
        [
            ...
            pl.col("Genre")
            .map_elements(get_primary_genre, return_dtype=pl.String)
            .alias("primary_genre"),
            pl.col("Genre")
            .map_elements(get_genre_count, return_dtype=pl.Int32)
            .alias("genre_count"),
        ]
    )
    ...
    df = df.with_columns(
        pl.col("Popularity")
        .map_elements(bin_popularity, return_dtype=pl.String)
        .alias("popularity_bin")
    )
