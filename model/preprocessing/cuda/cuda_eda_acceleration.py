...
class CUDASimulator:
    ...
    def run_parallel_reduction(self):
        ...
        print(
            f"CUDA Config: Block size = {block_size} threads, Grid size = {num_blocks} blocks."
        )
        ...
        print(
            f"Reduction Validation: {'SUCCESS' if np.allclose(cpu_sum, gpu_sum) else 'FAILED'}"
        )
    ...
    def run_monte_carlo_ssta(self, n_samples=10000):
        ...
        print(
            f"Simulating delays for {n_gates} timing gates with {n_samples} random samples each."
        )
        ...
        gpu_time = (datetime.now() - gpu_start).total_seconds() * 1000
        print(
            f"Parallel Box-Muller SSTA timing delay simulation finished in: {gpu_time:.3f} ms"
        )

        # Verify and print stats
        for g in range(n_gates):
            print(
                f"  Gate '{gate_types[g]}': Simulated Mean={np.mean(delays[g,:]):.3f}ns, StdDev={np.std(delays[g,:]):.3f}ns"
            )
        ...
        plt.title(
            "Croix & Khatri Case Study: Simulated SSTA Timing Delay Distributions (CUDA Box-Muller)"
        )
        plt.xlabel("Delay (ns)")
    ...
    def run_fault_simulation(self, n_vectors=10000):
        ...
        print(
            f"Evaluating {n_gates} logic gates across {n_vectors} fault test vectors in parallel."
        )
        ...
        if gpu_time_s > 0:
            print(
                f"Average simulated GPU gate throughput: {(n_gates * n_vectors) / gpu_time_s:,.0f} ops/sec"
            )
        else:
            print(
                "Average simulated GPU gate throughput: High-speed parallel throughput (under clock resolution)"
            )
