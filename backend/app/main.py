...
from recommendation import build_recommendation_matrix, get_recommendations
...
def get_titles(
    ...
    return {"total": total_count, "page": page, "limit": limit, "results": results}
...
@app.get("/recommend")
def recommend_titles(title: str, limit: int = 10, db: Session = Depends(get_db)):
...
def get_dashboard_aggregates(db: Session = Depends(get_db)):
    ...
    genre_avg_ratings = sorted(
        genre_avg_ratings, key=lambda x: float(x["avg"]), reverse=True
    )[:8]

    # 3. Year distribution of releases (e.g. content growth)
    year_distribution = (
        db.query(Movie.release_year, func.count(Movie.id).label("count"))
        .filter(Movie.release_year >= 1970)
    ...
    language_distribution = (
        db.query(Movie.original_language, func.count(Movie.id).label("count"))
    ...
    comparison = {}
    for p in platforms:
        comparison[p] = {"total": 0, "movies": 0, "tv_shows": 0, "avg_rating": 0.0}
...
def get_advanced_stats():
    ...
    results_path = os.path.join(
        root_dir, "..", "model", "evaluation", "ml_results.json"
    )
    if not os.path.exists(results_path):
        ...
        return {
            ...
            "association_rules": [
                ...
                {
                    "antecedent": "Science Fiction",
                    "consequent": "Adventure",
                    "support": 0.054,
                    "confidence": 0.541,
                    "lift": 3.12,
                },
...
def get_predictions_and_clustering(db: Session = Depends(get_db)):
    ...
    """
    movies = db.query(
        Movie.title, Movie.popularity, Movie.vote_average, Movie.primary_genre
    ).all()
    if not movies:
    ...
    df["popularity_capped"] = np.clip(
        df["popularity"], 0, df["popularity"].quantile(0.99)
    )
    ...
    if len(hist_years) > 2:
        ...
        for y, val in zip(future_years, pred_future):
            forecast_data.append(
                {"year": y, "actual": None, "forecast": max(0, int(round(val)))}
            )
    else:
        forecast_data = []
